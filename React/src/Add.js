...
function Add() {
  ...
  let handleSubmit = (e) => {
    ...
    if (result === true) {
      axios
        .post("http://localhost:6001/users", inputData)
        .then((res) => {
  ...
  const validateValues = (inputData) => {
    if (inputData.Name.length === 0) {
      ...
      return false;
    } else if (inputData.Flavour.length === 0) {
      alert("Please enter Flavour Name !!!");
      return false;
    ...
    >
      <div className="w-50 border bg-light p-5">
        <form onSubmit={handleSubmit}>
          <h2 id="align">Add User's Data</h2>
          <div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setInputData({ ...inputData, Flavour: e.target.value })
              }
            />
          </div>
