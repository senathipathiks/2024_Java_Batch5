...
function FindEmployee() {
  ...
  useEffect(() => { 
    axios
      .get("http://localhost:2222/employees/all")
      .then((response) => {
  ...
  const handleSearch = () => {
    axios
    .get("http://localhost:2222/employees/"+id)
    .then((response) => {
  ...
  let handleSubmit = (id) => {
    ...
    if (conf) {
      axios
        .delete("http://localhost:2222/employees/" + id)
        .then((res) => {
    ...
    return (
      ...
      <br />
      <div className="text-end">
        <Link to="/showpay" className="btn btn-primary">
          Show Payrolls
        </Link>
      </div>
      <br />
          <div className="text-end">
            <SearchInput type="number" id="id" name="id" onBlur={(e) => setId(e.target.value)}/>
            ...
            <tbody>
              {records.map((d, i) => (
                <TableRow key={i}>
                  ...
                  <TableCell>{d.payroll.netSalary}</TableCell>
